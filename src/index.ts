...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'GTEx');
}
...
const tools: McpToolExport['tools'] = [
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        gene: { type: 'string', description: 'Gene symbol (e.g. "TP53") or versioned Gencode id (e.g. "ENSG00000141510.16"). Symbols are resolved automatically.' },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'median_expression': {
      let gencodeId = reqStr(args, 'gene', '"TP53"');
      // GTEx matches only exact VERSIONED gencode ids (e.g. ENSG00000141510.16).
      // Resolve a symbol/unversioned id via the reference endpoint so "TP53"
      // works, not just the versioned id nobody memorizes.
      if (!/^ENSG\d+\.\d+$/i.test(gencodeId)) {
        const ref = (await gtGet(`/reference/gene?${new URLSearchParams({ geneId: gencodeId })}`)) as { data?: Array<{ gencodeId?: string }> };
        const resolved = ref?.data?.find((g) => g.gencodeId)?.gencodeId;
        if (resolved) gencodeId = resolved;
      }
      // datasetId is REQUIRED by the GTEx v2 API — without it medianGeneExpression
      // returns an empty data array even for a valid gencode id.
      const p = new URLSearchParams({ gencodeId, datasetId: (args.datasetId as string) || 'gtex_v8' });
