...
`tools/list` at `https://gateway.pipeworx.io/gtex/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/gtex_gene \
  -H 'Content-Type: application/json' \
  -d '{"gencode_id_or_symbol":"BRCA1"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/gtex_gene`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
